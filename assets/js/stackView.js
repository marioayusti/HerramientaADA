...
function pushStack(value) {
	if (thisGlobal.top < SIZE && value != "") {
		thisGlobal.implementAction(thisGlobal.push.bind(thisGlobal), value);
	}
}
		
function popStack() {
	if (thisGlobal.top > 0){
		thisGlobal.implementAction(thisGlobal.pop.bind(thisGlobal), '');
	}
}

function clearStack() {
	thisGlobal.implementAction(thisGlobal.clearAll.bind(thisGlobal), "");
}
...
StackLL.prototype.push = function(elemToPush) {
	...
	this.cmd("CreateLabel", labPushID, "Agregando Valor: ", PUSH_LABEL_X, PUSH_LABEL_Y);
...
function initStack(id) {
	var animManag = initCanvas(id,125);
